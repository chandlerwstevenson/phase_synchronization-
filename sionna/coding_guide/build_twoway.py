...
# %% SECTION: wrap\_phase
# %% NOTE: A phase is an angle: $370^\circ$ \emph{is} $10^\circ$. Every
# %% NOTE: phase subtraction in this program goes through this one-liner,
# %% NOTE: or differences near the boundary come out wrong by a full
# %% NOTE: turn. It appears on nearly every line that touches a phase;
# %% NOTE: when debugging this kind of code, suspect a missing wrap
# %% NOTE: first. (Repository: \texttt{core.py:24}.)
...
# %% SECTION: Two small linear-algebra helpers
# %% NOTE: \texttt{\_as\_matrix} is convenience. \texttt{\_covariance\_root}
# %% NOTE: matters: to draw correlated noise with covariance $Q$ you need
# %% NOTE: a matrix square root of $Q$; an eigendecomposition gives one
# %% NOTE: that tolerates positive-\emph{semi}definite $Q$ (a zero
# %% NOTE: eigenvalue = a noiseless direction), which a plain Cholesky
# %% NOTE: factorization would reject. (\texttt{core.py:50--62}.)
def _as_matrix(
...
# %% SECTION: resolve\_device
# %% NOTE: Runs on the graphics card when one is available, the
# %% NOTE: processor otherwise. (\texttt{core.py:133}.)
...
# %% SECTION: The Oscillator -- the enemy
# %% NOTE: The clock model: state $[\theta, \omega]$ (phase, frequency).
# %% NOTE: The transition matrix says phase advances by frequency
# %% NOTE: $\times$ time. The noise is a \textbf{random walk in both
# %% NOTE: components} -- drawn through the covariance root so the two
# %% NOTE: components are correctly correlated -- and random walks
# %% NOTE: accumulate; they never average away. \texttt{apply\_correction}
# %% NOTE: is the actuator the loop will drive; nothing else may touch
# %% NOTE: the state. Keep the transition matrix in mind: the Kalman
# %% NOTE: filter carries an exact copy of it, so filter and physics
# %% NOTE: agree by construction. (\texttt{core.py:175--225}.)
class Oscillator:
...
# %% SECTION: SDRSimulationConfig -- every physical assumption
# %% NOTE: One frozen dataclass holds every knob: timing (50\,ms
# %% NOTE: intervals, 1\,MHz sampling), the channel (3GPP model ``D'',
# %% NOTE: 100\,ns delay spread, 915\,MHz), the pilot structure
# %% NOTE: ($16\times16$ short, $2047\times2$ long), the oscillator noise
# %% NOTE: levels, every hardware impairment, and the loop's actuation
# %% NOTE: limits (16-bit phase corrections, one-interval command
# %% NOTE: latency, 1\,ms turnaround). Every experiment in the
# %% NOTE: repository is a variation of these fields -- never edited
# %% NOTE: code. (\texttt{sdr.py:33--94}.)
...
# %% NOTE: The dataclass rejects nonsense at construction: negative
# %% NOTE: noise levels, a cyclic prefix longer than its sequence, too
# %% NOTE: few training repeats. Fail loudly at setup, not silently
# %% NOTE: mid-run. The \texttt{sample\_period} property is the only
# %% NOTE: derived quantity. (\texttt{sdr.py:96--157}.)
...
# %% SECTION: Three small data carriers
# %% NOTE: \texttt{SyncPreamble}: the pilot waveform plus the layout
# %% NOTE: facts the receiver needs. \texttt{IQCapture}: what one
# %% NOTE: capture returns -- the impaired samples, an impairment-free
# %% NOTE: \emph{oracle} copy (ground truth for diagnostics), and the
# %% NOTE: end value of the intra-frame oscillator walk, which the loop
# %% NOTE: must carry into the clock state. \texttt{SDRMeasurement}:
# %% NOTE: what the receiver reports -- detected?, phase, frequency,
# %% NOTE: timing, and the detection metric. (\texttt{sdr.py:160--191}.)
@dataclass(frozen=True)
class SyncPreamble:
...
# %% SECTION: The pilot: Zadoff--Chu training fields
# %% NOTE: Constant-amplitude chirps with a single-spike
# %% NOTE: autocorrelation -- the same family LTE and 5G use. The
# %% NOTE: preamble is $16\times16$ short repeats (for coarse timing and
# %% NOTE: frequency) then $2047\times2$ long sequences with a cyclic
# %% NOTE: prefix (for fine frequency and phase). Why 2047: the
# %% NOTE: frequency-estimate error times the 50\,ms interval is phase
# %% NOTE: drift the loop must ride out, and it must stay far inside
# %% NOTE: the $\pm\pi/2$ branch boundary you will meet later.
# %% NOTE: (\texttt{sdr.py:253--288}.)
...
# %% SECTION: Converter and clock imperfections
# %% NOTE: \texttt{\_quantize\_iq}: a $b$-bit converter -- clip to full
# %% NOTE: scale, round to the grid, report the clip rate.
# %% NOTE: \texttt{\_soft\_limit}: the power amplifier's gentle
# %% NOTE: saturation. \texttt{\_resample\_clock\_offset}: the receiver's
# %% NOTE: sample clock runs at a slightly different rate than the
# %% NOTE: transmitter's, so the samples are linearly re-interpolated at
# %% NOTE: the offset rate plus a fractional delay.
# %% NOTE: (\texttt{sdr.py:290--325}.)
...
# %% SECTION: Flicker frequency noise
# %% NOTE: Real oscillators have $1/f$ frequency noise on top of the
# %% NOTE: white walks. The standard time-domain surrogate: a bank of
# %% NOTE: four first-order decaying processes with log-spaced time
# %% NOTE: constants, summed. The class also exposes its per-interval
# %% NOTE: \texttt{innovation\_variance} -- the loop adds it to the
# %% NOTE: filter's process noise, so the filter knows about this noise
# %% NOTE: source too. (\texttt{sdr.py:328--391}.)
...
# %% SECTION: SDRRadioLink: the channel
# %% NOTE: The propagation channel is Sionna's 3GPP tapped-delay-line,
# %% NOTE: converted to a 16-tap discrete-time filter, one realization
# %% NOTE: per sync interval. \textbf{\texttt{mirror\_of} is reciprocity
# %% NOTE: in one argument}: the reverse link of a two-way pair shares
# %% NOTE: the forward link's taps and shadowing -- the physical fact
# %% NOTE: the entire method stands on. The noise floor is fixed at the
# %% NOTE: nominal channel gain, so fading changes the actual
# %% NOTE: signal-to-noise ratio, as in the field.
# %% NOTE: (\texttt{sdr.py:394--473}.)
class SDRRadioLink:
...
# %% SECTION: Link helpers: jitter and shadowing
# %% NOTE: \texttt{\_random\_start}: the capture window lands with
# %% NOTE: random timing jitter. \texttt{\_channel\_for\_frame}: this
# %% NOTE: interval's taps, shaped for Sionna's channel operator.
# %% NOTE: \texttt{\_step\_shadowing}: slow log-normal fading as a
# %% NOTE: correlated process -- and the mirrored link reuses the
# %% NOTE: forward link's draw (reciprocity again).
# %% NOTE: (\texttt{sdr.py:475--508}.)
...
# %% SECTION: capture(), transmit side
# %% NOTE: One direction of one exchange, in physical order: the
# %% NOTE: receiver's sample-clock error moves the arrival window
# %% NOTE: (whole samples step it; the sub-sample residue stays as a
# %% NOTE: fractional delay); the transmitter imprints \emph{its}
# %% NOTE: oscillator on the waveform; the power amplifier soft-clips;
# %% NOTE: the DAC quantizes; Sionna's channel convolves; shadowing
# %% NOTE: scales. (\texttt{sdr.py:510--544}.)
    def capture(
...
# %% SECTION: capture(), receive side
# %% NOTE: Thermal noise at the fixed floor; sample-clock resampling;
# %% NOTE: down-conversion with the \emph{receiver's} oscillator (what
# %% NOTE: remains is the clock difference plus the channel); the
# %% NOTE: oscillator's own random walk \emph{during} the frame -- whose
# %% NOTE: end value is returned so the loop keeps the noise continuous
# %% NOTE: across frames; white phase jitter; IQ imbalance; automatic
# %% NOTE: gain control; DC offset; 12-bit quantization. Every phase
# %% NOTE: error the filter must fight is created here, physically.
# %% NOTE: (\texttt{sdr.py:545--618}.)
...
# %% SECTION: The receiver, stage 1: coarse timing and frequency
# %% NOTE: The receiver knows neither when the pilot arrives nor the
# %% NOTE: frequency offset. Correlating the signal with itself 16
# %% NOTE: samples later solves both at once: when the repeating short
# %% NOTE: field fills the window, consecutive repeats match -- the
# %% NOTE: normalized magnitude peaks (timing), and the angle between
# %% NOTE: repeats is the offset accumulated over 16 samples
# %% NOTE: (frequency, unambiguous to $\pm f_s/32 = \pm31$\,kHz).
# %% NOTE: (\texttt{sdr.py:621--659}.)
...
# %% SECTION: The receiver, stages 2--3: detect, refine, read the phase
# %% NOTE: Stage 2: after coarse derotation, the \emph{whole} preamble
# %% NOTE: is matched against every window; the normalized peak gives
# %% NOTE: precise timing, and its height is the detection metric --
# %% NOTE: below the 0.25 threshold the capture is declared failed and
# %% NOTE: the loop must coast. Stage 3: the two long fields are
# %% NOTE: identical and 2175 samples apart; the angle between them
# %% NOTE: refines the frequency to $\sim$0.1\,Hz. Finally, derotate
# %% NOTE: with the refined frequency (indices centered so the phase is
# %% NOTE: referenced to frame \emph{center}) and matched-filter the
# %% NOTE: long fields: the correlation angle is the carrier phase --
# %% NOTE: clock offset \textbf{plus the channel's phase}. Separating
# %% NOTE: those two is the loop's job. (\texttt{sdr.py:661--754}.)
...
# %% SECTION: R from physics, and quantized corrections
# %% NOTE: The filter's measurement covariance is \textbf{derived, not
# %% NOTE: tuned}: the textbook estimation bound for a pilot of this
# %% NOTE: length and signal-to-noise ratio, plus the phase noise the
# %% NOTE: oscillator smears across the frame, plus per-sample jitter.
# %% NOTE: This is why no gain is hand-tuned anywhere.
# %% NOTE: \texttt{\_quantize\_correction} snaps commands to what
# %% NOTE: hardware can apply: 16-bit phase steps, 0.01\,Hz frequency
# %% NOTE: steps. (\texttt{sdr.py:757--815}.)
...
# %% SECTION: The extended Kalman filter: state, predict, and the circle
# %% NOTE: The filter tracks the \emph{relative} state
# %% NOTE: $[\hat\theta, \hat\omega]$ with the same transition matrix as
# %% NOTE: the Oscillator. \texttt{predict}: push the estimate through
# %% NOTE: the drift model, inflate the covariance -- ``the clocks kept
# %% NOTE: drifting while I wasn't looking.'' The measurement is a
# %% NOTE: \textbf{point on the unit circle}, $[\cos\phi, \sin\phi,
# %% NOTE: \omega]$, so no innovation ever wraps; the price is a
# %% NOTE: nonlinear $h$, linearized by the Jacobian -- which is the
# %% NOTE: entire meaning of ``extended.'' (\texttt{core.py:286--339}.)
...
# %% SECTION: The iterated update, Joseph form, and the reset
# %% NOTE: Three deliberate choices. \textbf{Iterate}: with a large
# %% NOTE: offset one linearized shot under-corrects (the comment says
# %% NOTE: it best: $\sin(-1.4)$ is not $\approx -1.4$) -- so update,
# %% NOTE: re-linearize, repeat until converged. \textbf{Joseph form}:
# %% NOTE: the textbook covariance shortcut can go asymmetric or
# %% NOTE: negative under rounding; this form cannot.
# %% NOTE: \textbf{reset\_after\_correction}: after commanding the
# %% NOTE: clock to shift, subtract the command from the estimate --
# %% NOTE: the filter tracks \emph{what remains}. Skip it and every
# %% NOTE: correction applies twice. (\texttt{core.py:341--382}.)
...
# %% SECTION: The result object
# %% NOTE: Everything the run records, with the honest masks built in:
# %% NOTE: \texttt{steady\_state\_phase\_rms} and
# %% NOTE: \texttt{mean\_coherent\_gain} only count intervals where
# %% NOTE: \texttt{detected \& correction\_active \& calibrated} --
# %% NOTE: steady state means \emph{after} acquisition, actuation, and
# %% NOTE: the $\pi$ check, and the code says so explicitly.
# %% NOTE: (\texttt{coherent.py:42--101}.)
...
# %% SECTION: \_pick\_half\_phase: the two candidate answers
# %% NOTE: The two-way trick divides a phase difference by two, and
# %% NOTE: division by two on a circle loses one bit: the true offset
# %% NOTE: is either the measurement or the measurement $+\pi$ --
# %% NOTE: \emph{exactly} two candidates, at any signal-to-noise ratio.
# %% NOTE: Once locked, the offset cannot physically jump by $\pi$ in
# %% NOTE: one interval, so the filter's own prediction picks the
# %% NOTE: branch. At first contact there is no prediction -- that hole
# %% NOTE: is plugged by the one-time power check inside the loop.
# %% NOTE: (\texttt{coherent.py:104--119}.)
...
# %% SECTION: run\_two\_way\_simulation: setup
# %% NOTE: Seeds (note \texttt{sionna\_config.seed} -- the channel is
# %% NOTE: pinned by Sionna's own seed, not torch's; a real trap), two
# %% NOTE: oscillators sharing one covariance, the reverse link as a
# %% NOTE: \texttt{mirror\_of} the forward link, and the filter's
# %% NOTE: matrices assembled from physics: $R$ = the derived
# %% NOTE: covariance \emph{halved} (the half-difference averages two
# %% NOTE: independent captures); $Q$ = \emph{twice} the per-oscillator
# %% NOTE: covariance (two independent clocks make the relative state)
# %% NOTE: + the white-FM walk over one interval + the flicker
# %% NOTE: innovation. Initial covariance: phase $\pm\pi$, frequency
# %% NOTE: $\pm50$\,kHz -- honest ignorance. Airtime is accounted
# %% NOTE: exactly: two captures per interval.
# %% NOTE: (\texttt{coherent.py:122--231}.)
...
# %% SECTION: The loop: drift, delayed corrections, the pi check
# %% NOTE: Each interval starts with physics: both clocks walk; the
# %% NOTE: master absorbs last frame's carried oscillator walk and the
# %% NOTE: flicker step. A correction scheduled
# %% NOTE: \texttt{correction\_latency\_intervals} ago lands now. Then
# %% NOTE: the \textbf{one-time $\pi$ calibration}: the acquisition
# %% NOTE: branch was a guess (wrong about half the time, with the pair
# %% NOTE: transmitting in opposite phase while every signal reads
# %% NOTE: ``locked''); three settled corrections after lock, one
# %% NOTE: coarse combined-power check -- destructive $\Rightarrow$
# %% NOTE: flip by $\pi$, once. The comment explains why the filter
# %% NOTE: needs no reset: the measurement cannot see a $\pi$ shift.
# %% NOTE: (\texttt{coherent.py:233--264}.)
...
# %% SECTION: The loop: two captures and the gap between them
# %% NOTE: Bookkeeping first: the correction-free ``physical'' clock
# %% NOTE: (what hardware would do with no synchronization at all)
# %% NOTE: yields the sample-clock offset for the captures. Then the
# %% NOTE: forward capture; its intra-frame oscillator walk joins the
# %% NOTE: true state \emph{before} the reverse frame fires -- one
# %% NOTE: continuous noise process. Between directions sits a real
# %% NOTE: 1\,ms transmit/receive turnaround: both clocks advance at
# %% NOTE: their current frequencies and pick up random walk. Then the
# %% NOTE: reverse capture, and the leftover walk for the rest of the
# %% NOTE: interval is drawn and carried to the next iteration.
# %% NOTE: (\texttt{coherent.py:266--318}.)
...
# %% SECTION: The loop: the half-difference, acquire, track
# %% NOTE: The central lines of the method. Forward measures
# %% NOTE: $\theta_m-\theta_s+\varphi_{ch}$; reverse measures
# %% NOTE: $\theta_s-\theta_m+\varphi_{ch}$; half the difference
# %% NOTE: cancels the channel exactly (reciprocity). The turnaround
# %% NOTE: drift is backed out with the loop's \emph{own} frequency
# %% NOTE: estimate. Then the filter: predict; tell it about a landed
# %% NOTE: correction; on first detection seed the state from the
# %% NOTE: measurement; afterwards pick the branch against the
# %% NOTE: prediction and update on the circle. No detection
# %% NOTE: $\Rightarrow$ coast. (\texttt{coherent.py:320--363}.)
...
# %% SECTION: The loop: actuate, record, and the figure of merit
# %% NOTE: The correction that will land $L$ intervals from now should
# %% NOTE: cancel the offset \emph{then} -- so the estimate is pushed
# %% NOTE: $L$ steps through the transition matrix, quantized to what
# %% NOTE: hardware can apply, and scheduled; the filter reset happens
# %% NOTE: when it \emph{lands}. The scoreboard is the true residual
# %% NOTE: between the clocks -- something only a simulation can see --
# %% NOTE: and the coherent gain it implies:
# %% NOTE: $|1+e^{j\,\text{residual}}|^2/4 = \cos^2(\text{residual}/2)$.
# %% NOTE: (\texttt{coherent.py:364--420}.)
